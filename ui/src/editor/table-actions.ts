...
import {
  BetweenHorizontalEnd, BetweenHorizontalStart, BetweenVerticalEnd, BetweenVerticalStart, PanelTop, TableColumnsSplit, TableRowsSplit, Trash2, type LucideIcon,
} from "lucide-react";
...
export const TABLE_ACTIONS: TableAction[] = [
  { id: "row-above", title: "Zeile darüber", icon: BetweenHorizontalStart, keywords: "tabelle zeile row oben einfügen", run: (c) => c.addRowBefore() },
  ...
  { id: "row-delete", title: "Zeile löschen", icon: TableRowsSplit, keywords: "tabelle zeile row entfernen löschen", danger: true, run: (c) => c.deleteRow() },
  ...
  { id: "header", title: "Kopfzeile umschalten", icon: PanelTop, keywords: "tabelle kopfzeile header überschrift", run: (c) => c.toggleHeaderRow() },
...
export const runTableAction = (editor: Editor, a: TableAction) => a.run(editor.chain().focus()).run();
